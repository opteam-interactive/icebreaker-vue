...
const addTestParticipants = () => {
    participants.value = (["Nathan", "Luc", "Clément", "Loic", "Louanne"]);
}
...
<template>
    <div class="flex flex-col items-center h-full justify-center gap-8">
        ...
        <div class="grid md:grid-cols-5 md:flex-row gap-4 w-3/4 md:w-2/3 mx-auto">
            <input type="text" placeholder="Nom du participant" class="input rounded-full col-span-2 w-full " v-model="newName" />
            <button class="btn btn-success rounded-full btn-soft" @click="addParticipant(newName)">Ajouter <UserPlus/></button>
            <button class="btn btn-info rounded-full btn-soft" @click="reorderParticipants">Réorganiser <ArrowDownUp/></button>
            <button class="btn btn-error rounded-full btn-soft" @click="clearParticipants">Reinitialiser <CircleX/> <ListRestart/> </button>
        ...
        <div v-if="participants.length === 0" class="flex flex-col items-center my-16  justify-center gap-8 ">
            <h1 class="text-4xl">Aucun participant pour le moment</h1>
        </div>

        <div v-else class="border w-3/4 rounded-xl">
            <div class="grid md:grid-cols-2 max-h-96 overflow-scroll">
        ...
        <RouterLink to="/game"
            class="btn rounded-full  bg-lightblue  lg:w-1/2 gap-4">
            JOUER <Dices />
        </RouterLink>
    </div>
</template>
